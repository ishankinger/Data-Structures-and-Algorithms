...
// USE OF RECURSION CONCEPTS
// SWAP EVERY TWO ADJACENT NODE OF THE LINKED LIST
// NOT TO MODIFY THE NODE DATA
...
Node* swapAdj(Node* &head){
    ...
    head->next=swapAdj(secNode->next);
    secNode->next=head;
    return secNode;
}
...
int main(){
    LinkedList ll;
    for(int i=0;i<10;i++) ll.insertAtTail(i);
    ll.display();
    LinkedList llAns;
    llAns.head=swapAdj(ll.head);
    llAns.display();
    return 0;
}
