...
void insertAtFirst(Node*&head, int value){       // FUNCTION TO INSERT ELEMENT AT FIRST POSITION
    Node *n= new Node(value);                    // MAKING A NEW NODE
    n->next=head;                                // POINTING THIS NODE NEXT TO HEAD OF OUR GIVEN LINKED LIST
    head=n;                                      // AND CHANGING OUR HEAD TO NEW NODE
}
void insertAtTail(Node * &head, int value){      // WE ARE GETTING HEAD POINTER WHICH DIRECTLY GIVE US OUR LINKED LIST
...
void insert(Node* &head,int value,int pos){
    if(pos==0){                                  // IF POS==0 MEANS FIRST POSITION
        insertAtFirst(head,value);               // SO WE WILL BE USING OUR INSERT AT FIRST FUNCTION
...
void update(Node* &head,int value,int pos){      // FUNCTION TO UPDATE THE VALUE AT KTH POSITION
...
void display(Node *&head){                       // FUNCTION TO DISPLAY ELEMENTS OF THE LINKED LIST
...
bool search(Node*&head, int key){                // FUNCTION TO CHECK WHETHER GIVEN ELEMENT IS PRESENT OR NOT IN THE LINKED LIST
...
void deleteElem(Node* &head, int value){         // FUNCTION TO DELETE ELEMENT OF GIVEN VALUE
...
void deleteHead(Node* &head){                    // TO DELETE ELEMENT AT HEAD
    Node *toDelete=head;                         // STORE HEAD NOE
    head=head->next;                             // MAKING NEXT NODE TO HEAD AS NEW HEAD
    delete toDelete;                             // DELETE HEAD NODE
}
void deleteTail(Node* &head){                    // FUNCTION TO DELETE ELEMENT AT LAST POSITION                 
...
int main(){
    Node *head=NULL;                             // MAKING A NULL NODE AS HEAD
    for(int i=0;i<5;i++) insertAtFirst(head,i);  // NOW ADDING ELEMENTS TO NULL NODE
    display(head);
    for(int i=0;i<5;i++) insertAtTail(head,i);
    display(head);
    insert(head,5,5);
    display(head);
    update(head,6,5);
