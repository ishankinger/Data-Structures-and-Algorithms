// GETTING MIDDLE ELEMENT OF THE LINKED LIST
// ONE METHOD IS TO DO USING LENTH FUNCTION BUT IT WILL TAKE TWO TRAVERSAL
// BUT WE WANT TO DO THIS IN ONE TRAVERSAL

// SO WE WILL BE USING *****SLOW-FAST POINTER APPROACH***** TO GET THE MIDDLE ELEMENT
// ONE POINTER WILL BE MOVING TWICE ELEMENTS IN ONE RUN AND SLOW POINTER WILL BE MOVING ONE ELEMENT
// SO WHEN FAST POINTER WILL REACH THE END, THE SLOW POINTER WILL BE PRESENT AT THE MIDDLE ELEMENT
...
Node* getMidEle(Node* &head){
    ...
    while(ptr_fast!=NULL && ptr_fast->next!=NULL){     // FOR EVEN FAST SHOULD BE EQUAL TO NULL AND FOR ODD FAST NEXT TO BE NULL
        ptr_fast=ptr_fast->next->next;                 // FAST MOVE BY TWO NODES EACH TIME
        ptr_slow=ptr_slow->next;                       // SLOW MOVE BY ONE NODE EACH TIME
    }
    return ptr_slow;
}
...
int main(){
    ...
    Node* midNode=getMidEle(ll1.head);
    cout<<midNode->data<<"\n";
