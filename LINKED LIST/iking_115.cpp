// *************** GROUPING ODD-EVEN INDICES TOGETHER *****************
// GIVEN A LINKED LIST, GROUP ALL THE ODD INDICES OF LINKED LIST TOGETHER AND EVEN INDICES AFTER THAT
...
Node* oddEvenList(Node* head) {                                     // another easy  function for the same
        if(head == NULL || head->next == NULL) return head;
        Node* toContinue = head->next;
        Node* save;
        Node* temp = head;
        ...
        temp->next = toContinue;
        return head;
}


Node* groupOddAndEven(Node* &head){              // FUNCTION TO ODD AND THEN EVEN INDICES
    Node* temp=head;
    Node* secEle=head->next;                     // STORING SEC ELEMENT HEAD WHICH TO BE USED FURTHER 
    int len=0;                                   // STORING LEN TO GET TWO CONDITIONS POSSIBLE
    Node* save;                                  // HELP IN SINGLE TAVERSAL ONE BY ONE ON ALL ELEEMENTS
    while(temp->next->next){                     // MOVE TILL SEC LAST ELEMENT
        save=temp->next;                         // SAVE TEMP NEXT SO TO USE FOR NEXT ELEMENT TURN
        temp->next=temp->next->next;             // POINT TO ODD POSITION SKIPPING ONE ELEMENT
        temp=save;                               // NEXT ELEMENT WILL BE SAVED ONE AS IT'S NEXT IS CHANGED
        len++;                                   // ALSO KEEP COUNT OF LENGTH
    }
    if(len%2==0) save->next=secEle;              // THESE TWO CASES CAN BE UNDERSTOOD FROM WRITTING THEM
    else{
        save->next->next=secEle;
        save->next=NULL;
    }
    return head;
}
int main(){
    ...
    ll.display();
    ll.head = groupOddAndEven(ll.head);
    ll.display();
    return 0;
}
