...
class Employee{                                           // A CLASS IS DEFINED USING 'CLASS' KEYWORD AND THEN NAME OF THE CLASS
    ...
    public:                                               // IN THIS ANY VARIABLE CAN BE USED DIRECTLY AND HAVE NO RESTRICTIONS ON IT
        ...
        void setData(int a,int b,int c);                  // DECLARATION OF ONE FUNCTION OF THE CLASS -> Setter Funcion

        void getData(){                                   // ANOTHER FUNCTION WHICH HAS SOME CODE WRITTEN INSIDE AND NOT ONLY DECLARED
            cout<<"The value of a is "<<a<<endl;          // -> Getter Function
...
// USING THIS LABEL WE CAN GIVE SAME NAME TO FUNCTIONS OF DIFFERENT CLASSES AS IT WILL BE RESOLVED FROM THEIR LABEL CLASS
...
// (FUNCTION'S CODE IS DIRECTLY INSERTED AT EVERY CALL SITE BY THE COMPILER, RATHER THAN EXECUTING A SEPARATE FUNCTION CALL)
...
int main(){
    ...
    // getData();                                          // MEMBER FUNCTION CAN BE CALLED ONLY VIA OBJECT OF CLASS OR OTHER MEMBER FUNCTIONS
