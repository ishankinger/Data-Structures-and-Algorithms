...
int main(){
    ...
    // Here’s where your overloaded operator-> comes in.
    // T->num is interpreted as T.operator->()->num.
    // Your operator->() returns this, i.e., a test* pointer to T.
    // Then ->num accesses the num member of the returned pointer.
    // The compiler will recursively call operator->() if the returned type also has operator->, until it finally gets a raw pointer.
