...
// NESTING MEANS IN ANY MEMBER FUNCTIONS WE CAN USE THAT FUNCTION IN OTHER FUNCTION ( BY CALLING THAT FUNCTION BY IT'S NAME DIRECTLY )
...
void Binary :: chk_bin(void){                              // FUNCTION TO CHECK WHETHER THE STRING CONTAINS OF ONLY BINARY NO. THAT IS ONLY
    ...
    if(k==(int)s.length()){                                    // SO IF FOR WHOLE TRAVERSAL IT IS FINE THEN IT IS A BINARY NUMBER
...
void Binary :: ones(void){                                 // FUNCTION TO NOT THE GIVEN BINARY NUMBER 
    ...
    for(int i=0;i<(int)s.length();i++){                          // MEANS IF IT IS 1 MAKE IT 0 AND VICE VERSA
        if(s.at(i)=='0'){
            cout<<'1';
        }else if(s.at(i)=='1'){
             cout<<'0';
