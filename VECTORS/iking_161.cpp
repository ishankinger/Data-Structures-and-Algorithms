...
// GIVEN Q QUERIES AND IN THAT CHECK WHETHER NUMBER IS PRESENT IN ARRAY OR NOTZ( VALUE OF ALL ELEMENTS ARE LESS THAN 10^5 )
...
int main(){
    ...
    for(int i=0;i<6;i++){                        // LOOPING IN THE GIVEN VECTOR AND VALUE OF ELEMENT IS ACTING AS AN INDEX FOR THE FREQUENCY ARRAY
        freq[v[i]] +=1;                          // AS MANY TIMES IT COMES IT WILL BE ADDED BY ONE SO WE GET FREQUENCY OF EACH ELEMENT WITH THIS
    }
